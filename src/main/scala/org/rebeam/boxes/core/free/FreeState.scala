...
/** An `EffectWorld` implemented in terms of `Trampoline`. */
trait FixedWorld extends EffectWorld {

  class Action[A] protected[FixedWorld] (private[FixedWorld] val t: State => Trampoline[(State, A)]) {
    def map[B](f: A => B): Action[B] =
      new Action(w => for { p <- t(w) } yield (p._1, f(p._2)))
    def flatMap[B](f: A => Action[B]): Action[B] =
...
object MapWorld extends FixedWorld with App {
  type State = Map[String, String]
